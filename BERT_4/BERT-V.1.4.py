...
history = model.fit(
    ...
    validation_data=([val_input_ids, val_attention_masks], df_val['Sentiment'].values),  # Validation data
    epochs=EPOCHS,
    batch_size=BATCH_SIZE
)

# Save the model for future use
model.save('BERT_V.1.4.h5')
# Predict the class labels directly from the BERT model
result_bert = model.predict([test_input_ids, test_attention_masks])
y_pred_labels = np.argmax(result_bert, axis=1)  # Convert predictions to class labels

# Generate the classification report
print('\tClassification Report for BERT:\n\n',
      classification_report(y_test, y_pred_labels, target_names=['Negative', 'Neutral', 'Positive', 'Irrelevant']))
...
# y_test is already in categorical form, so no need to apply np.argmax to y_test
...
plt.title('Confusion Matrix for BERT-4 Sentiment Analysis')
